...
def get_depth_values(batch, D, level, device, depth, std, near_far):
    ...
    v_s = cfg.credsplatting.cas_config.volume_scale[level]
    ...
    if depth is None:
        ...
        up_scale = cfg.credsplatting.cas_config.volume_scale[level] / cfg.credsplatting.cas_config.volume_scale[level-1]
        if up_scale != 1.:
        ...
        if cfg.credsplatting.cas_config.depth_inv[level-1]:
...
def build_rays_composite(depth, std, batch, training, near_far, level, up_scale=2., xywh=None):
    device = depth.device
    up_scale = cfg.credsplatting.cas_config.render_scale[level] / cfg.credsplatting.cas_config.volume_scale[level]
    if up_scale != 1.:
    ...
    x, y, w, h = (xywh * cfg.credsplatting.cas_config.render_scale[level]).int()
...
def build_feature_volume(feature, batch, D, depth, std, near_far, level):
    ...
    proj_mats = get_proj_mats(batch, src_scale=cfg.credsplatting.cas_config.im_feat_scale[level], tar_scale=cfg.credsplatting.cas_config.volume_scale[level])
    volume_sum = 0
    volume_sq_sum = 0
    for s in range(S):
        feature_s = feature[:, s]
...
def build_feature_volume2(feature, batch, D, depth, std, near_far, level, tgt_fea=None, omega_net=None):
    B, S, C, H, W = feature.shape
    depth_values, near_far = get_depth_values(batch, D, level, feature.device, depth, std, near_far)
    proj_mats = get_proj_mats(batch, src_scale=cfg.credsplatting.cas_config.im_feat_scale[level], tar_scale=cfg.credsplatting.cas_config.volume_scale[level])
    
    paired_feature_volume = []
    paired_reweight = []
    ...
    if tgt_fea is None:
        ...
        for s in range(S):
            ...
            warped_volume = warped_volume_src * tgt_volume

            # paired 
            pair_volume = torch.cat((warped_volume, warped_volume_src),dim=1)
            paired_feature_volume.append(pair_volume)
            # warped_volume = warped_volume.permute(0,2,1,3,4).squeeze(0)
            warped_volume = rearrange(warped_volume, 'b c d h w -> (b d) c h w')
            ...
            warped_volume = rearrange(warped_volume, '(b d) c h w -> b c d h w', b = B, d = D)
            reweight = rearrange(reweight, '(b d) c h w -> b c d h w', b = B, d = D)
            paired_reweight.append(reweight) # [b, c, d, h, w]

            if warped_volumes is None:
                warped_volumes = (reweight + 1) * warped_volume
                volume_context = (reweight + 1) * warped_volume_src
                weight_sum = (reweight + 1)
            else:
                warped_volumes = warped_volumes + (reweight + 1) * warped_volume
                volume_context = volume_context + (reweight + 1) * warped_volume_src
                weight_sum = weight_sum + (reweight + 1)
                
        volume_variance = warped_volumes / weight_sum
        volume_context = volume_context / weight_sum
        # volume_variance = warped_volumes / S
        # volume_context = volume_context / S
        feature_volume = torch.cat((volume_variance, volume_context), dim=1)

    return feature_volume, depth_values, near_far, paired_feature_volume, paired_reweight


from .save_img import save_feature_heatmap, save_feature_heatmap3, save_feature_heatmap4
import sys
def build_feature_volume3(feature, batch, D, depth, std, near_far, level, tgt_fea=None, omega_net=None):
    B, S, C, H, W = feature.shape
    depth_values, near_far = get_depth_values(batch, D, level, feature.device, depth, std, near_far)
    proj_mats = get_proj_mats(batch, src_scale=cfg.credsplatting.cas_config.im_feat_scale[level], tar_scale=cfg.credsplatting.cas_config.volume_scale[level])
    ...
    if tgt_fea is None:
        ...
        # feature_volume = volume_context.clone()
        feature_volume = volume_variance.clone()
        ...
        # warped_volumes_t = None
        # volume_context_t = None
        # volume_add_src_t = None
        # volume_pow_src_t = None
        ...
        for s in range(S):
            ...
            proj_mat = proj_mats[:, s]
            warped_volume_src, _ = homo_warp(feature_s, proj_mat, depth_values)
            warped_volume2 = (warped_volume_src - tgt_volume).pow_(2)
            warped_volume = warped_volume_src * tgt_volume
            # print(torch.unique(warped_volume), torch.unique(warped_volume_src), torch.unique(tgt_volume))
            # print(torch.unique(warped_volume2))
            #得到两种表达
            # paired
            # pair_volume = torch.cat((warped_volume, warped_volume_src),dim=1)
            pair_volume = torch.cat((warped_volume, warped_volume2), dim=1)
            paired_feature_volume.append(pair_volume)
            # warped_volume = warped_volume.permute(0,2,1,3,4).squeeze(0)
            # print(warped_volume.shape)
            warped_volume = rearrange(warped_volume, 'b c d h w -> (b d) c h w')
            # print(warped_volume.shape)
            # dist.barrier()
            # print(f"Rank {dist.get_rank()} passed the barrier at omega_net start, {S}")
            reweight = omega_net(warped_volume)
            # dist.barrier()
            # print(f"Rank {dist.get_rank()} passed the barrier at omega_net end, {S}")
            # warped_volume = warped_volume.permute(1,0,2,3).unsqueeze(0)
            # reweight = reweight.permute(1,0,2,3).unsqueeze(0)
            warped_volume = rearrange(warped_volume, '(b d) c h w -> b c d h w', b = B, d = D)
            reweight = rearrange(reweight, '(b d) c h w -> b c d h w', b = B, d = D)
            ...
            # if save_fig:
            #     # save_feature_heatmap(warped_volume[0,0,0,...].cpu().numpy(), f"./img/warped_volume_{s}.png")
            #     # save_feature_heatmap(warped_volume2[0,0,0,...].cpu().numpy(), f"./img/warped_volume2_{s}.png")
            #     save_feature_heatmap4(reweight[0,0,4,...].cpu().numpy(), f"./img/reweight_{s}.png", flag=True)
            #     # print(src_inps[0,s,...].permute(1,2,0).shape)
            #     save_feature_heatmap3(src_inps[0,s,...], f"./img/src_inps_{s}.png", flag=False)
            #     # save_feature_heatmap((reweight * warped_volume)[0,0,0,...].cpu().numpy(), f"./img/reweigh*warped_volume_{s}.png")
            #     # save_feature_heatmap(warped_volume_src[0,0,0,...].cpu().numpy(), f"./img/warped_volume_src_{s}.png")
            #     # save_feature_heatmap((reweight * warped_volume_src)[0,0,0,...].cpu().numpy(), f"./img/reweigh*warped_volume_src_{s}.png")

            # reweight = 1.0
            if warped_volumes is None:
                warped_volumes = (reweight + 0) * warped_volume
                volume_context = (reweight + 0) * warped_volume_src
                weight_sum = (reweight + 0)
                volume_add_src = warped_volume_src
                volume_pow_src = warped_volume_src ** 2
                
                # warped_volumes_t =  warped_volume
                # volume_context_t = warped_volume_src
                # weight_sum_t = 1.0
                # volume_add_src_t = warped_volume_src
                # volume_pow_src_t = warped_volume_src ** 2
            else:
                warped_volumes = warped_volumes + (reweight + 0) * warped_volume
                volume_context = volume_context + (reweight + 0) * warped_volume_src
                weight_sum = weight_sum + (reweight + 0)
                volume_add_src = volume_add_src + warped_volume_src
                volume_pow_src = volume_pow_src + warped_volume_src ** 2

                # warped_volumes_t = warped_volumes_t + warped_volume
                # volume_context_t = volume_context_t +  warped_volume_src
                # weight_sum_t = weight_sum_t + 1 
                # volume_add_src_t = volume_add_src_t + warped_volume_src
                # volume_pow_src_t = volume_pow_src_t + warped_volume_src ** 2
                
        volume_variance = warped_volumes / weight_sum
        volume_context = volume_context / weight_sum
        # volume_context_src = volume_add_src / S
        volume_variance_src = volume_pow_src.div_(S).sub_(volume_add_src.div_(S).pow_(2))

        # volume_variance_t = warped_volumes_t / weight_sum_t
        # volume_context_t = volume_context_t / weight_sum_t
        # volume_variance_src_t = volume_pow_src_t.div_(S).sub_(volume_add_src_t.div_(S).pow_(2))
        # if save_fig:
        #     save_feature_heatmap4(volume_variance[0,0,4,...].cpu().numpy(), f"./img/volume_variance.png")
        #     save_feature_heatmap4(volume_context[0,0,4,...].cpu().numpy(), f"./img/volume_context.png")
        #     # # save_feature_heatmap(volume_variance_src[0,0,4,...].cpu().numpy(), f"./img/volume_variance_src.png")

        #     # save_feature_heatmap(volume_variance_t[0,0,0,...].cpu().numpy(), f"./img/volume_variance_t.png")
        #     # save_feature_heatmap(volume_context_t[0,0,0,...].cpu().numpy(), f"./img/volume_context_t.png")
        #     # # # # save_feature_heatmap(volume_variance_src_t[0,0,4,...].cpu().numpy(), f"./img/volume_variance_src_t.png")

        #     # save_feature_heatmap((volume_variance_t - volume_variance)[0,0,0,...].cpu().numpy(), f"./img/volume_variance_c.png")
        #     # save_feature_heatmap((volume_context_t-volume_context)[0,0,4,...].cpu().numpy(), f"./img/volume_context_c.png")
        #     # save_feature_heatmap((volume_variance_src_t-volume_variance_src)[0,0,4,...].cpu().numpy(), f"./img/volume_variance_src_c.png")
        # dist.barrier()
        # print(f"Rank {dist.get_rank()} passed the barrier at build_feature_volume12 start")
        feature_volume = torch.cat((volume_variance, volume_context, volume_variance_src), dim=1)
        # sys.exit()
    return feature_volume, depth_values, near_far, paired_feature_volume, paired_reweight

def build_feature_volume_net(feature, batch, D, depth, std, near_far, level, net):
    B, S, C, H, W = feature.shape
    depth_values, near_far = get_depth_values(batch, D, level, feature.device, depth, std, near_far)
    proj_mats = get_proj_mats(batch, src_scale=cfg.credsplatting.cas_config.im_feat_scale[level], tar_scale=cfg.credsplatting.cas_config.volume_scale[level])
    volume_sum = 0
    ...
    for s in range(S):
        ...
        proj_mat = proj_mats[:, s]
        warped_volume, _ = homo_warp(feature_s, proj_mat, depth_values)
        volume_sum = volume_sum + net(warped_volume)
        volume_sq_sum = volume_sq_sum + warped_volume ** 2
    volume_variance = volume_sq_sum.div_(S).sub_(volume_sum.div_(S).pow_(2))
    volume_context = volume_sum.div_(S)
    feature_volume = torch.cat((volume_variance, volume_context),dim=1)
...
def get_img_feat(xyz, img_feat_rgb, batch, training, level):# B * N * S * (11+4)
    ...
    render_scale = cfg.credsplatting.cas_config.render_scale[level]
    ...
    for i in range(S):
        ...
        grid = grid * 2. - 1.
        # print(grid.shape, img_feat_rgb.shape)
        feat = F.grid_sample(img_feat_rgb[:, i], grid[:, None], align_corners=True, mode='bilinear', padding_mode='border').permute(0, 2, 3, 1)[:, 0]
        # print(F.grid_sample(img_feat_rgb[:, i], grid[:, None], align_corners=True, mode='bilinear', padding_mode='border').permute(0, 2, 3, 1).shape)
        ...
        # print(feat.shape, ray_diff.shape)
        ret_feat.append(torch.cat([feat, ray_diff], dim=-1))
    # print(torch.stack(ret_feat, -2).shape)
...
def parse_layer(layer, batch, hw, level, layer_idx):
    ...
    H, W = hw
    H, W = int(H*cfg.credsplatting.cas_config.render_scale[level]), int(W*cfg.credsplatting.cas_config.render_scale[level])
    x, y, w, h = (batch['bbox'][0][layer_idx] * cfg.credsplatting.cas_config.render_scale[level]).int()
    x, y, w, h = x.item(), y.item(), w.item(), h.item()
