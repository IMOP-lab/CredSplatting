...
from .feature_net import FeatureNet, AutoEncoder, AutoEncoder2
...
# from lib.gaussian_renderer_dpt import render
# from lib.gaussian_renderer_liu import render
import os
import imageio
import numpy as np
import PIL
...
from lib.vggt.models.aggregator import Aggregator
from lib.vggt.heads.dpt_head2 import DPTHead
...
class Network(nn.Module):
    def __init__(self,):
        ...
        self.omega = AutoEncoder2(16, 4, 4)
        # self.sp = InterViewAAModule(32)
        self.flag = False
        # self.omega = AutoEncoder(16, 24, 16)

        self.vggt_aggregator = Aggregator(img_size=518, patch_size=14, embed_dim=1024)
        # state_dict_vggt = self.vggt_aggregator.state_dict()

        weight_path = "/home/lab2/2T/CredSplatting/model.pt"
        state_dict = torch.load(weight_path, map_location=torch.device('cpu'))  # 可以指定设备，如'cuda'
        # 筛选出键中包含'aggregator'的键值对
        new_state_dict = {key.replace('aggregator.', ''): value for key, value in state_dict.items() if 'aggregator' in key}

        # 加载权重到模块
        self.vggt_aggregator.load_state_dict(new_state_dict)

        self.dec_head = DPTHead(dim_in=2 * 1024, output_dim=2, activation="exp", conf_activation="expp1")

        # 增加批归一化和激活函数，使特征提取更稳定
    ...
    def forward_feat_vggt(self, x, true_shape):
        B, S, C, _, _ = x.shape
        H, W = true_shape
        with torch.no_grad(): #先试试不反向传播
            aggregated_tokens_list, patch_start_idx = self.vggt_aggregator(x)
        out_dict = self.dec_head(
                aggregated_tokens_list, images=x, patch_start_idx=patch_start_idx
            )
        
        feat2 = F.interpolate(out_dict[3], size=(H, W), align_corners=True, mode='bilinear')
        feat1 = F.interpolate(out_dict[2], size=(H // 2, W // 2), align_corners=True, mode='bilinear')
        feat0 = F.interpolate(out_dict[1], size=(H // 4, W // 4), align_corners=True, mode='bilinear')
        feat2 = self.dec_conv2(feat2)
        feat1 = self.dec_conv1(feat1)
        feat0 = self.dec_conv0(feat0)
        # print('1')
        feats = {
                'level_2': feat2.reshape((B, S, feat2.shape[1], H, W)),
                'level_1': feat1.reshape((B, S, feat1.shape[1], H//2, W//2)),
                'level_0': feat0.reshape((B, S, feat0.shape[1], H//4, W//4)),
                }
        return feats
    ...
    def forward(self, batch):
        ...
        if not cfg.save_video:
            ...
            feats = self.forward_feat_vggt(batch['src_inps2'], batch['true_shape'])
            ret = {}
            #=========================== Stage_0 init ====================================
            ...
            # print(depth.shape, feature_volume.shape, batch['src_inps'].shape)
            ret_0 = {}
            ...
            gs_novel0 = []
            render_novel0 = []
            # print(world_xyz.shape, rot_out.shape, scale_out.shape, opacity_out.shape, color_out.shape, rgb_vr.shape, depth2.shape)
            for b_i in range(B):
            ...
            for fvp, reweight in zip(paired_feature_volume, paired_reweight):
                ...
                # print(feats[f'level_{im_feat_level}'].shape)
            ...
            if cfg.save_ply:
                ...
                for b_i in range(B):
                    ...
                    depth_minmax = [
                        batch["near_far"].min().detach().cpu().numpy()/cfg.credsplatting.scale_factor,
                        batch["near_far"].max().detach().cpu().numpy()/cfg.credsplatting.scale_factor,
                    ]
            ...
            for v_i, meta in enumerate(batch['rendering_video_meta']):
                ...
                for i in range(cfg.credsplatting.cas_config.num):
                    H, W = int(H_img*cfg.credsplatting.cas_config.render_scale[i]), int(W_img*cfg.credsplatting.cas_config.render_scale[i])
                    feature_volume, depth_values, near_far = utils.build_feature_volume(
                            feats[f'level_{i}'],
                            batch,
                            D=cfg.credsplatting.cas_config.volume_planes[i],
                            depth=depth,
                            std=std,
                            near_far=near_far,
                            level=i)
                    ...
                    im_feat_level = cfg.credsplatting.cas_config.render_im_feat_level[i]
                    ...
                    if i == cfg.credsplatting.cas_config.num-1:
                        ...
                        for b_i in range(B):
                            render_novel_i_0 = render(meta, b_i, world_xyz[b_i], color_out[b_i], rot_out[b_i], scale_out[b_i], opacity_out[b_i], bg_color=cfg.credsplatting.bg_color)
                            if cfg.credsplatting.reweighting: 
