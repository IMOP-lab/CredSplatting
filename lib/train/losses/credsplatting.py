...
class NetworkWrapper(nn.Module):
    ...
    def forward(self, batch):
        # flops, params = profile(self.net, (batch,))
        ...
        for i in range(cfg.credsplatting.cas_config.num):
            ...
            loss += cfg.credsplatting.cas_config.loss_weight[i] * color_loss
           
            psnr = -10. * torch.log(color_loss) / torch.log(torch.Tensor([10.]).to(color_loss.device))
            scalar_stats.update({f'psnr_{i}': psnr})
          
            num_patchs = cfg.credsplatting.cas_config.num_patchs[i]
            if cfg.credsplatting.cas_config.train_img[i]:
                ...
                loss += 0.05 * perceptual_loss * cfg.credsplatting.cas_config.loss_weight[i]
                scalar_stats.update({f'perceptual_loss_{i}': perceptual_loss.detach()})
                ssim = SSIM(window_size = 7)
                ssim_loss = 1-ssim(inp, tar)
                loss += 0.1 * ssim_loss * cfg.credsplatting.cas_config.loss_weight[i]
                scalar_stats.update({f'ssim_loss_{i}': ssim_loss.detach()})
                
            elif num_patchs > 0:
                patch_size = cfg.credsplatting.cas_config.patch_size[i]
                num_rays = cfg.credsplatting.cas_config.num_rays[i]
                ...
                loss += 0.01 * perceptual_loss * cfg.credsplatting.cas_config.loss_weight[i]
                scalar_stats.update({f'perceptual_loss_{i}': perceptual_loss.detach()})
