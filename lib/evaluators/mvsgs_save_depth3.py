        ...
        for i in range(cfg.credsplatting.cas_config.num):
            ...
            for b in range(B):
                ...
                if cfg.save_result and i == cfg.credsplatting.cas_config.num-1:
                    ...
                    # 保存 pred_rgb 图像
                    pred_img_path = os.path.join(cfg.result_dir, '{}_{}_{}_pred.png'.format(batch['meta']['scene'][b], batch['meta']['tar_view'][b].item(), batch['meta']['frame_id'][b].item()))
                ...
                if cfg.credsplatting.eval_depth and (i == cfg.credsplatting.cas_config.num - 1):
                    ...
                    # img_path = os.path.join(cfg.result_dir, 'depth_{}_{}_{}.png'.format(batch['meta']['scene'][b], batch['meta']['tar_view'][b].item(), batch['meta']['frame_id'][b].item()))
                    ...
                    # 保存 pred_depth 图像
                    pred_depth_img_path = os.path.join(cfg.result_dir, 'depth_{}_{}_{}_pred.png'.format(batch['meta']['scene'][b], batch['meta']['tar_view'][b].item(), batch['meta']['frame_id'][b].item()))
                    # 将 pred_depth 映射到彩色图 (0-255)
                    pred_depth_img = np.clip(pred_depth_img, 425., 905.)
                    pred_depth_color = colormap((pred_depth_img - 425.) / (905. - 425.))[:, :, :3]  # 去掉 alpha 通道
                    imageio.imwrite(pred_depth_img_path, (pred_depth_color * 255.).astype(np.uint8))

                    # # 如果需要将 gt_depth 和 pred_depth 水平拼接的结果保存为一张图片
                    # concatenated_depth_img = img_utils.horizon_concate(gt_depth_img, pred_depth_img)
                    # concatenated_depth_img_path = os.path.join(cfg.result_dir, 'depth_{}_{}_{}_concat.png'.format(batch['meta']['scene'][b], batch['meta']['tar_view'][b].item(), batch['meta']['frame_id'][b].item()))
                    # # 将拼接后的图像映射到彩色图 (0-255)
                    # concatenated_depth_color = colormap((concatenated_depth_img - 425.) / (905. - 425.))[:, :, :3]  # 去掉 alpha 通道
                    # imageio.imwrite(concatenated_depth_img_path, (concatenated_depth_color * 255.).astype(np.uint8))
