...
def build_rays(tar_img, tar_ext, tar_ixt, tar_msk, level, split):
    scale = cfg.credsplatting.cas_config.render_scale[level]
    if scale != 1.:
    ...
    if split == 'train' and not cfg.credsplatting.cas_config.train_img[level]:
        if cfg.credsplatting.sample_on_mask: # 313
            msk_sample = tar_msk
            num_fg_rays = int(min(cfg.credsplatting.cas_config.num_rays[level]*0.75, tar_msk.sum()*0.95))
            non_zero = msk_sample.nonzero()
        ...
        num_rays = cfg.credsplatting.cas_config.num_rays[level] - num_fg_rays
        X = np.random.randint(low=0, high=W, size=num_rays)
        Y = np.random.randint(low=0, high=H, size=num_rays)
        if num_fg_rays > 0:
            X = np.concatenate([X, X_]).astype(np.int32)
            Y = np.concatenate([Y, Y_]).astype(np.int32)
        if cfg.credsplatting.cas_config.num_patchs[level] > 0:
            X_, Y_ = sample_patch(cfg.credsplatting.cas_config.num_patchs[level], cfg.credsplatting.cas_config.patch_size[level], H, W, msk_sample)
            X = np.concatenate([X, X_]).astype(np.int32)
            Y = np.concatenate([Y, Y_]).astype(np.int32)
        num_rays = len(X)
