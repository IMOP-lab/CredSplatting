...
class Dataset:
    ...
    def build_metas(self):
        ...
        for scene in scenes:
            ...
            # print(train_ids, len(scene_info['exts']))
            ...
            for tar_view in render_ids:
                ...
                input_views_num = cfg.credsplatting.train_input_views[1] if self.split == 'train' else cfg.credsplatting.test_input_views
    ...
    def __getitem__(self, index_meta):
        ...
        # print(depth_ranges)
        near_far = np.array([1.5 * depth_ranges[:, 0].min().item(), 1.5 * depth_ranges[:, 1].max().item()]).astype(np.float32)
