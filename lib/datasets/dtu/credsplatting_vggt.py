...
class Dataset:
    ...
    def __getitem__(self, index_meta):
        ...
        src_inps, src_inps2, src_exts, src_ixts = self.read_src_vggt(scene_info, src_views)
    ...
    def read_src_vggt(self, scene_info, src_views, target_size=518):
        inps, inps2, exts, ixts = [], [], [], []

        for src_view in src_views:
            # 读取图像
            img_path = scene_info['img_paths'][src_view]
            ...
            inps2.append(img_normalized)
            inps.append((np.array(imageio.imread(scene_info['img_paths'][src_view])) / 255.) * 2. - 1)
            exts.append(scene_info['exts'][src_view])
            intrinsic = scene_info['ixts'][src_view].copy()
            ixts.append(intrinsic)

        # 调整数组形状并转换数据类型
        inps_np = np.stack(inps).transpose((0, 3, 1, 2)).astype(np.float32)
        inps2_np = np.stack(inps2).transpose((0, 3, 1, 2)).astype(np.float32)
        exts_np = np.stack(exts)
        ixts_np = np.stack(ixts)

        return inps_np, inps2_np, exts_np, ixts_np
