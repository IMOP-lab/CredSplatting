...
def load_colmap_data(realdir):
    ...
    sorted_keys = sorted(imdata.keys(), key=lambda k: imdata[k].name)
    # 创建排序后的字典
    imdata = {k: imdata[k] for k in sorted_keys}
...
def save_poses(basedir, poses, pts3d, perm):
    ...
    for i in perm:
        ...
        # print(len(zs))
        if len(zs) == 0:
            close_depth, inf_depth = 0., 8
        else:
            close_depth, inf_depth = np.percentile(zs,.1), np.percentile(zs, 99.9)
            close_depth = max(0.0, close_depth)
        # print( i, close_depth, inf_depth )
        
        save_arr.append(np.concatenate([poses[..., i].ravel(), np.array([close_depth, inf_depth])], 0))
        # save_arr.append(np.concatenate([poses[..., i].ravel(), np.array([0., 1.])], 0))
